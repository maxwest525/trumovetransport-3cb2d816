...
export default function AgentPayments() {
  ...
  const [form, setForm] = useState({
    customerName: "", customerEmail: "", amount: "", description: "Moving services",
    cardNumber: "", cardExpiry: "", cardCvc: "", routingNumber: "", accountNumber: "",
  });
  ...
  const handleReset = () => {
    setMode(null);
    setIsComplete(false);
    setForm({ customerName: "", customerEmail: "", amount: "", description: "Moving services", cardNumber: "", cardExpiry: "", cardCvc: "", routingNumber: "", accountNumber: "" });
  };

  return (
    <AgentShell breadcrumb=" / Payments">
      <div className="p-6 max-w-md mx-auto">
        ...
        {isComplete ? (
          <div className="py-6 text-center space-y-4">
            ...
            <Badge variant="outline" className="text-xs gap-1"><FileText className="w-3 h-3" />Receipt #INV-{Math.floor(Math.random() * 9999).toString().padStart(4, "0")}</Badge>
            <div className="pt-2"><Button variant="outline" onClick={handleReset}>New Payment</Button></div>
          </div>
        ) : !mode ? (
          <div className="space-y-4">
            <div className="space-y-3">
              <div className="space-y-1.5"><Label className="text-xs">Customer Name</Label><Input value={form.customerName} onChange={e => updateField("customerName", e.target.value)} placeholder="John Smith" /></div>
              <div className="space-y-1.5"><Label className="text-xs">Customer Email</Label><Input type="email" value={form.customerEmail} onChange={e => updateField("customerEmail", e.target.value)} placeholder="john@email.com" /></div>
              ...
              <div className="space-y-1.5"><Label className="text-xs">Description</Label><Input value={form.description} onChange={e => updateField("description", e.target.value)} placeholder="Moving services" /></div>
            </div>
            <Separator />
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button variant="outline" className="h-auto py-5 flex-col gap-2 hover:border-primary hover:bg-primary/5" onClick={() => setMode("charge")} disabled={!form.amount}>
                <CreditCard className="w-6 h-6" /><span className="text-sm font-semibold">Charge Now</span><span className="text-[10px] text-muted-foreground">Process card/ACH payment</span>
              </Button>
              <Button variant="outline" className="h-auto py-5 flex-col gap-2 hover:border-primary hover:bg-primary/5" onClick={() => setMode("invoice")} disabled={!form.amount}>
                <Receipt className="w-6 h-6" /><span className="text-sm font-semibold">Send Invoice</span><span className="text-[10px] text-muted-foreground">Email invoice to pay later</span>
              </Button>
            </div>
          </div>
        ) : mode === "charge" ? (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button variant={paymentMethod === "card" ? "default" : "outline"} size="sm" className="flex-1 gap-1.5" onClick={() => setPaymentMethod("card")}><CreditCard className="w-3.5 h-3.5" />Card</Button>
              <Button variant={paymentMethod === "ach" ? "default" : "outline"} size="sm" className="flex-1 gap-1.5" onClick={() => setPaymentMethod("ach")}><DollarSign className="w-3.5 h-3.5" />ACH</Button>
            </div>
            {paymentMethod === "card" ? (
              <div className="space-y-3">
