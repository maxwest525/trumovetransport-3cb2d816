...
// Trust Badge Strip
export const TrustBadgeStrip = ({ theme = 'light', brandColors }: { theme?: 'light' | 'dark'; brandColors?: BrandColors }) => {
  const badges = [
      ...
      {badges.map((badge, i) => (
        <div key={i} className="flex items-center gap-2">
          <badge.icon
            ...
            style={brandColors?.primary ? { color: brandColors.primary } : undefined}
          />
          <span
...
export const ThreeStepProcess = ({ theme = 'light', brandColors }: { theme?: 'light' | 'dark'; brandColors?: BrandColors }) => {
  const steps = [
  ...
  return (
    <div className={`py-16 px-8 ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`} style={sectionBg}>
      ...
      <div className="relative max-w-4xl mx-auto">
        <div
          className="absolute top-16 left-0 right-0 h-0.5 bg-gradient-to-r from-blue-500 via-blue-400 to-blue-500 hidden md:block"
          style={accentColor ? { background: `linear-gradient(to right, ${accentColor}, ${accentColor}80, ${accentColor})` } : undefined}
        />
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {steps.map((step, i) => (
            <div key={i} className="relative text-center">
              <div
                className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-3xl shadow-lg shadow-blue-500/30 relative z-10"
                style={accentColor ? { background: `linear-gradient(to bottom right, ${accentColor}, ${brandColors?.primaryDark || accentColor})`, boxShadow: `0 10px 15px -3px ${accentColor}30` } : undefined}
              >
                {step.icon}
              </div>
...
export const TripleGuaranteeSection = ({ brandColors }: { brandColors?: BrandColors } = {}) => {
  const guarantees = [
    ...
    <div className="py-16 px-8 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="text-center mb-12">
        <div
          ...
          style={accentColor ? { background: `linear-gradient(to bottom right, ${accentColor}, ${brandColors?.primaryDark || accentColor})`, boxShadow: `0 10px 15px -3px ${accentColor}40` } : undefined}
        >
...
export const VideoTestimonialGrid = ({ brandColors }: { brandColors?: BrandColors } = {}) => {
  const testimonials = [
            ...
            <div className="aspect-video bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center text-6xl relative">
              ...
              <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-lg">
                  <Play className="w-6 h-6 ml-1" style={accentColor ? { color: accentColor } : { color: '#2563EB' }} />
                </div>
              </div>
            </div>
            <div className="p-4">
              ...
              <p className="font-bold text-slate-900" style={textMain}>{t.name}</p>
              <p className="text-sm text-slate-500" style={textSub}>{t.location}</p>
            </div>
          </div>
        ))}
      </div>
...
export const ComparisonTableSection = ({ brandColors }: { brandColors?: BrandColors } = {}) => {
  const features = [
            ...
            {features.map((f, i) => (
              <tr key={i} className={i % 2 === 0 ? 'bg-slate-50' : 'bg-white'}>
                <td className="p-4 font-medium text-slate-900" style={textMain}>
                 ...
                 <td className="p-4 text-center" style={{ background: `${accentColor}08` }}>
                   {f.trumove ? <CheckCircle2 className="w-6 h-6 mx-auto" style={{ color: accentColor }} /> : <span className="text-slate-300">—</span>}
                </td>
...
export const FAQSection = ({ brandColors }: { brandColors?: BrandColors } = {}) => {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            ...
            <div className="space-y-2 text-sm text-slate-400">
              <p className="hover:text-white cursor-pointer" style={accentColor ? { color: undefined } : undefined}>Privacy Policy</p>
